...
type Member = { fullName: string; email: string; phone: string; track: string; experience?: string; age?: string };

export default function DelegationRegisterPage() {
  const [members, setMembers] = useState<Member[]>([ 
    { fullName: "", email: "", phone: "", track: "", experience: "beginner", age: "" },
    { fullName: "", email: "", phone: "", track: "", experience: "beginner", age: "" }
  ]);
  ...
  function addMember() { if (members.length < 40) setMembers((m) => [...m, { fullName: "", email: "", phone: "", track: tracks[0]?.value || "", experience: "beginner", age: "" }]); }
  ...
  function importBulk() {
    ...
    for (const line of lines) {
      ...
      parsed.push({ fullName: name, email: emailCol, phone: phoneCol, track: committeeCol ? matchTrack(committeeCol) : (tracks[0]?.value || ""), age: "" });
    }
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload: Record<string, unknown> = {
      ...
      members: members.filter((m) => m.fullName.trim()).map((m) => ({ fullName: m.fullName.trim(), email: m.email.trim(), phone: m.phone.trim(), track: m.track, age: m.age ? Number(m.age) : undefined }))
    };
            ...
            <div className="mt-2 space-y-2">
              {members.map((m, i) => (
                <div key={i} className="grid grid-cols-12 gap-2">
                  <input value={m.fullName} onChange={(e) => setM(i, "fullName", e.target.value)} placeholder={`Delegate ${i + 1} name`} className="col-span-3 rounded-lg border border-ink/15 bg-cream px-3 py-2 text-sm outline-none focus:border-gold" />
                  ...
                  <input value={m.age} onChange={(e) => setM(i, "age", e.target.value)} placeholder="Age" className="col-span-1 rounded-lg border border-ink/15 bg-cream px-2 py-2 text-sm outline-none focus:border-gold" />
