...
export default function Hero() {
  return (
    <section className="relative overflow-hidden bg-[#FFE8C8] min-h-[85vh] flex flex-col justify-between pt-44 pb-12 text-ink">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col items-center px-5 text-center my-auto">

        {/* Kicker */}
        <p className="kicker mb-4 text-center text-xs font-bold uppercase text-[#B45309] animate-fadeUp tracking-[0.2em] sm:tracking-[0.25em]">
          We built the summit we wished existed
        </p>

        {/* Bolder, Large Headline (sized nicely to fit the sky space) */}
        <h1
          className="relative z-10 w-full text-center font-display font-black text-[#2A1005] leading-[0.95] tracking-tight animate-fadeUp drop-shadow-[0_4px_10px_rgba(42,16,5,0.12)]"
        ...
        {/* Date badge */}
        <div className="mt-7 flex w-full max-w-md flex-wrap items-center justify-center gap-x-3 gap-y-1 rounded-full border-2 border-[#D97706]/40 bg-[#1F0A02] px-5 py-2.5 text-center text-[10px] font-bold tracking-[0.12em] text-[#FFF8E7] shadow-xl shadow-[#1F0A02]/35 animate-fadeUp sm:max-w-none sm:px-6 sm:text-xs sm:tracking-[0.18em]">
          <span className="h-2 w-2 shrink-0 rounded-full bg-[#F59E0B] animate-pulse" />
          <span>22nd – 23rd AUGUST 2026 · NEW DELHI</span>
        </div>

        {/* Countdown */}
        <div className="relative z-10 w-full select-none scale-[0.85] min-[400px]:scale-90 sm:scale-100">
          <CountdownTimer />
        </div>

        {/* CTA buttons */}
        <div className="mt-10 flex w-full max-w-sm flex-col items-center justify-center gap-4 sm:max-w-none sm:flex-row animate-fadeUp relative z-10">
          <Link
            href="/register"
