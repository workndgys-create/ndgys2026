...
export default function CompetitionRegisterForm({ competition: c, slug }: { competition: Comp; slug?: string }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload: Record<string, unknown> = {
      ...
      teamChoice: isIplAuction ? (fd.get("teamChoice") || "") : undefined,
    ...
    <form onSubmit={onSubmit} className="mt-8 space-y-4 rounded-2xl border border-ink/10 bg-paper p-7 shadow-sm">
      ...
      {isIplAuction && (
        <div>
          ...
          <select name="teamChoice" className="mt-1 w-full rounded-lg border border-ink/15 bg-cream px-3 py-2.5 outline-none focus:border-gold">
