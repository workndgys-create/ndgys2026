...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const guide = await prisma.committeeGuide.findUnique({ where: { id: params.id } });
  if (!guide || guide.trackSlug !== me.trackSlug) return NextResponse.json({ error: "Not found" }, { status: 404 });
