...
function parseOptionalNumber(value: unknown) {
  if (value === "" || value === null || value === undefined) return { ok: true as const, value: null };
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if ("order" in b) {
    ...
    if (!parsed.ok) return NextResponse.json({ error: "Order must be a valid number." }, { status: 422 });
    data.order = parsed.value ?? 0;
  }
  if ("date" in b) {
    const parsed = parseOptionalDate(b.date);
    if (!parsed.ok) return NextResponse.json({ error: "Date must be a valid date/time." }, { status: 422 });
  ...
  if ("minTeam" in b) {
    ...
    if (!parsed.ok) return NextResponse.json({ error: "Min team size must be a valid number." }, { status: 422 });
    data.minTeam = parsed.value;
  }
  if ("maxTeam" in b) {
    const parsed = parseOptionalNumber(b.maxTeam);
    if (!parsed.ok) return NextResponse.json({ error: "Max team size must be a valid number." }, { status: 422 });
    data.maxTeam = parsed.value;
  }
  if ("feeSolo" in b) {
    const parsed = parseOptionalNumber(b.feeSolo);
    if (!parsed.ok) return NextResponse.json({ error: "Solo fee must be a valid number." }, { status: 422 });
    data.feeSolo = parsed.value;
  }
  if ("feeGroup" in b) {
    const parsed = parseOptionalNumber(b.feeGroup);
    if (!parsed.ok) return NextResponse.json({ error: "Group fee must be a valid number." }, { status: 422 });
  ...
  try {
    ...
    console.error("[admin/competitions/:id PATCH]", { id: params.id, body: b, error });
    if (error?.code === "P2025") return NextResponse.json({ error: "Competition not found." }, { status: 404 });
    if (error?.code === "P2002") return NextResponse.json({ error: "A competition with that unique value already exists." }, { status: 409 });
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    console.error("[admin/competitions/:id DELETE]", { id: params.id, error });
    if (error?.code === "P2025") return NextResponse.json({ error: "Competition not found." }, { status: 404 });
    return NextResponse.json({ error: "Could not delete competition." }, { status: 500 });
  }
}
