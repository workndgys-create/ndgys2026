...
export async function GET(req: NextRequest) {
  ...
  if (id) {
    ...
    const pdf = await generateBadgePdf({ delegateId: r.delegateId, fullName: r.fullName, trackName: r.trackName, trackSlug: r.trackSlug, portfolio: r.portfolio });
  ...
  const list: BadgeData[] = (regs as unknown as { delegateId: string; fullName: string; trackName: string; trackSlug: string; portfolio: string | null }[])
    .map((r) => ({ delegateId: r.delegateId, fullName: r.fullName, trackName: r.trackName, trackSlug: r.trackSlug, portfolio: r.portfolio }));
