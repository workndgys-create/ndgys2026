...
const shell = (inner: string) => `
  <div style="font-family:Georgia,'Times New Roman',serif;background:#F7F4EC;padding:32px">
    <div style="max-width:560px;margin:auto;background:#fff;border:1px solid #e7e2d4;border-radius:14px;overflow:hidden">
      <div style="background:#1A1A3E;color:#E4C97E;padding:20px 28px;font-size:17px;letter-spacing:.4px;font-weight:bold">
        NEW DELHI GLOBAL YOUTH SUMMIT 2026
      </div>
      <div style="padding:28px;color:#222;line-height:1.6;font-size:15px">${inner}</div>
      <div style="padding:16px 28px;background:#FBFAF5;color:#6B6E8A;font-size:12px">
...
export const templates = {
  registrationPaid: (name: string, track: string, delegateId: string, amountPaise: number, qrDataUrl: string) =>
    shell(
      `<h2 style="margin:0 0 12px">You're confirmed, ${name}. 🎉</h2>
       <p>Your seat at the <b>New Delhi Global Youth Summit 2026</b> is secured.</p>
       <table style="margin:14px 0;font-size:14px">
         <tr><td style="color:#6B6E8A;padding:2px 12px 2px 0">Delegate ID</td><td><b>${delegateId}</b></td></tr>
         <tr><td style="color:#6B6E8A;padding:2px 12px 2px 0">Track</td><td>${track}</td></tr>
         <tr><td style="color:#6B6E8A;padding:2px 12px 2px 0">Amount paid</td><td>${inr(amountPaise)}</td></tr>
       </table>
       ...
       <p style="color:#6B6E8A;font-size:13px">Access your delegate dashboard anytime at ${env.NEXT_PUBLIC_BASE_URL}/dashboard</p>`
    ),
  magicLink: (link: string, otp: string) =>
    shell(
      `<h2 style="margin:0 0 12px">Sign in to your delegate dashboard</h2>
       <p>Use the button below, or enter this one-time code:</p>
       <p style="font-size:30px;letter-spacing:8px;font-weight:bold;color:#1A1A3E;margin:16px 0">${otp}</p>
       <p style="margin:20px 0"><a href="${link}" style="background:#C9A24B;color:#1A1A3E;padding:12px 24px;border-radius:999px;text-decoration:none;font-weight:bold">Open Dashboard →</a></p>
       <p style="color:#6B6E8A;font-size:13px">This link and code expire in 15 minutes. If you didn't request this, ignore this email.</p>`
    ),
  waitlistConfirm: (name: string, track: string) =>
    shell(
      `<h2 style="margin:0 0 12px">You're on the waitlist, ${name}.</h2>
  ...
  announcement: (title: string, body: string) =>
    shell(`<h2 style="margin:0 0 12px">${title}</h2><p style="white-space:pre-wrap">${body}</p>`),
  adminNewRegistration: (name: string, email: string, track: string) =>
    shell(`<h3 style="margin:0 0 8px">New paid registration</h3><p><b>${name}</b> (${email})<br/>Track: ${track}</p>`),
  adminNewContact: (name: string, email: string, subject: string, message: string) =>
    shell(`<h3 style="margin:0 0 8px">New contact message</h3><p><b>${name}</b> (${email})<br/><b>Subject:</b> ${subject}</p><p style="white-space:pre-wrap">${message}</p>`),
  refundProcessed: (name: string, track: string, amountPaise: number) =>
    shell(`<h2 style="margin:0 0 12px">Your registration has been cancelled</h2><p>Hi ${name}, your <b>${track}</b> registration was cancelled and a refund of ${inr(amountPaise)} has been initiated.</p>`),
  ...
  competitionConfirmed: (name: string, competition: string, participation: string, refId: string, amountPaise: number, teamName?: string | null) =>
    shell(`
      <h2 style="font-family:Georgia,serif;color:#1A1A3E;margin:0 0 8px">You're registered!</h2>
  ...
  adminNewCompetition: (name: string, email: string, competition: string, participation: string) =>
    shell(`
      <h3 style="font-family:Georgia,serif;color:#1A1A3E;margin:0 0 8px">New competition entry</h3>
  ...
  waitlistPromoted: (name: string, track: string, link: string) =>
    shell(`
      <h2 style="font-family:Georgia,serif;color:#1A1A3E;margin:0 0 8px">A seat just opened!</h2>
      <p style="color:#444">Hi ${name}, a place has opened in <b>${track}</b>. Seats are first-come — register now to claim it.</p>
      <p style="margin:18px 0"><a href="${link}" style="background:#C9A24B;color:#10112B;padding:12px 22px;border-radius:999px;text-decoration:none;font-weight:600">Register now</a></p>
    `),

  abandonedRegistration: (name: string, track: string, link: string) =>
    shell(`
      <h2 style="font-family:Georgia,serif;color:#1A1A3E;margin:0 0 8px">Almost there, ${name}</h2>
      ...
      <p style="margin:18px 0"><a href="${link}" style="background:#C9A24B;color:#10112B;padding:12px 22px;border-radius:999px;text-decoration:none;font-weight:600">Complete registration</a></p>
  ...
  delegationConfirmed: (headName: string, school: string, count: number, amountPaise: number) =>
    shell(`
      <h2 style="font-family:Georgia,serif;color:#1A1A3E;margin:0 0 8px">Delegation confirmed</h2>
  ...
  backgroundGuideUploaded: (name: string, title: string, committee: string, link: string) =>
    shell(`
      <h2 style="font-family:Georgia,serif;color:#1A1A3E;margin:0 0 8px">New background guide available</h2>
      ...
      <p style="margin:18px 0"><a href="${link}" style="background:#C9A24B;color:#10112B;padding:12px 22px;border-radius:999px;text-decoration:none;font-weight:600">View in dashboard</a></p>
    `)
};
