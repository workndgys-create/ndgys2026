    ...
    <section id="events" className="bg-cream grain py-24">
      <div className="mx-auto max-w-6xl px-5">
        <Reveal>
          ...
          <h2 className="mt-5 text-center font-display text-4xl font-700 text-ink sm:text-left sm:text-6xl">WHAT'S <span className="text-gold">ON.</span></h2>
        </Reveal>
        <div className="mt-12 grid items-stretch gap-6 md:grid-cols-2">
          {items.map((e: any, i: number) => (
            <Reveal key={e.id} delay={(i % 2) * 90} className="h-full">
              <article className="flex h-full min-h-[180px] gap-5 rounded-2xl border border-ink/10 bg-paper p-5 transition hover:-translate-y-1 hover:shadow-xl">
                {e.imageUrl ? (
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                  ...
                  <p className="mt-2 flex-1 text-sm text-ink/70">{e.summary}</p>
                </div>
              </article>
            </Reveal>
          ))}
